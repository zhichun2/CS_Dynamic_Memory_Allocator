...
#define LINENUM(i)                                                             \
    (i + HDRLINES + 1) /* cnvt trace request nums to linenums (origin 1) */
...
static char *default_giant_tracefiles[] = {DEFAULT_GIANT_TRACEFILES, NULL};
...
static void run_tests(int num_tracefiles, const char *tracedir,
                      char **tracefiles, stats_t *mm_stats,
                      speed_t *speed_params)
{
    ...
    for (i = 0; i < num_tracefiles; i++)
    {
        ...
        else
        {
			...
			mm_stats[i].valid = mm_stats[i].valid &&
				eval_mm_valid(trace, ranges);
...
int main(int argc, char **argv)
{
    ...
    if (run_libc)
    {
        ...
        {
            trace_t *trace =
                read_trace(&libc_stats[i], tracedir, global_tracefiles[i]);
        ...
        if (verbose)
        {
            ...
            printresults(num_global_tracefiles, libc_stats,
                         &global_libc_sum_stats);
        }
    }
    ...
    min_throughput = ref_throughput * (checkpoint ? MIN_SPEED_RATIO_CHECKPOINT
                                                  : MIN_SPEED_RATIO);

    max_throughput = ref_throughput * (checkpoint ? MAX_SPEED_RATIO_CHECKPOINT
                                                  : MAX_SPEED_RATIO);
    ...
    if (run_libc)
    {
        printf("Comparison with libc malloc: mm/libc = %.0f Kops / %.0f Kops = "
               "%.2f\n",
               (float)global_mm_sum_stats.tput,
               (float)global_libc_sum_stats.tput,
    ...
    /*
     * Calulate harmonic mean throughput. Since we do not know the number of
    ...
    if (globalUsage > 128)
    {
        spaceDeduct = (globalUsage - 128) / 8.0;
        if (spaceDeduct > 20)
            spaceDeduct = 20;
    ...
    {
        sprintf(autoresult,
                "{\"scores\": {\"Autograded Score\": %.1f}, \"scoreboard\": "
                "[%.1f, %.0f, %.0f, %.1f]}",
                score - spaceDeduct, score, -1.0 * spaceDeduct,
                avg_mm_harm_throughput, avg_mm_util * 100);
        printf("%s\n", autoresult);
    }
    exit(0);
}
                      ...
                      const trace_t *trace, int opnum, int index)
{
    ...
    if (!IS_ALIGNED(lo))
    {
        malloc_error(trace, opnum,
                     "Payload address (%p) not aligned to %d bytes", lo,
                     ALIGNMENT);
        return false;
    }
...
static bool check_index(const trace_t *trace, int opnum, int index)
{
    ...
    if (ngarbled != 0)
    {
        malloc_error(trace, opnum,
                     ...
                     "starting at byte %zu",
                     index, &block[firstgarbled], ngarbled, randint_t_name,
                     (ngarbled > 1 ? "s" : ""),
...
static void printresults(int n, stats_t *stats, sum_stats_t *sumstats)
{
    ...
    for (i = 0; i < n; i++)
    {
        if (stats[i].valid)
        {
            ...
            if (tab_mode)
            {
                printf("%.0f\t%.3f\t%.0f\t", stats[i].ops, msecs, kops);
            }
            else
            {
 ...
 * Give template suitable for use with sprintf, with one entry suitable for an
 * integer value. E.g., "./file_%.8x.txt".
 */
